...
Promotion deletes an entry only when the issue can actually carry it. Issues hold no requirements (rule 4), so an Epic absorbs a one-line idea but not concrete detail — the specific list of items and rhythms that an Epic's one sentence was distilled *from*, for instance. Detail like that stays parked until a change folder exists to receive it, and is deleted when the spec that supersedes it is archived. An entry outliving its Epic is expected; an entry outliving the spec that covers it is stale.
